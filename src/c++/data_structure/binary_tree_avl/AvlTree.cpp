...
/**
 * @info chave a ser inserida na árvore
 **/
bool AvlTree::insert(int info)
{
	bool h = false; //Indica se houve ou não alteração na altura da subárvore

	return doInsert(info, root, h);
}

/**
 * @info chave a ser inserida
 * @ptr ponteiro da árvore que está sendo vizitada
 * @h indica se houve alteração na árvore
 **/
bool AvlTree::doInsert(int info, Node*& ptr, bool& h)
{
	...
	if(ptr == 0)
    {
		ptr = new Node(info); //Cria um novo nó
		numberOfNodes++; //Incrementa a quantidade de nós
		h = true;

		return true; //Retorna true pois foi realizada a inserção
	}

	//Verifica se o valor a ser inserido já está na Árvore
	if (ptr->info == info)
        return false;

    bool ret = false;

    if (info < ptr->info) // Inserção será realizada na subárvore da esquerda
	{
        ...
        if(h)
        {
            switch(ptr->balance)
            {
                case 1: // Se balance era 1, passa a ser 0, logo não precisa atualizar balanceamento de ninguém acima
                    ptr->balance = 0;
                    h = false;
                    break;
                case 0: // Se balance era 0, passa a ser -1, então continua atualizando os balances acima
                    ptr->balance = -1;
                    break;
                case -1: // Se balance era -1, passa a ser -2, logo ele executa uma rotação para a direita
                    rightRotation(ptr);
                    h = false; // Devido a rotação, o altura volta a ter o valor anterior
            }
        }
	}
	else // Inserção será realizada na subárvore da direita
    {
...
/**
 * @pt ponteiro da raiz da subárvore que está desbalanceada
 **/
void AvlTree::rightRotation(Node*& pt)
{
    Node* ptu = pt->left;

    if (ptu->balance == -1) // Inserção feita a esquerda, logo a rotação é simples a direita
    {
		//Rotação à direita
        pt->left = ptu->right;
        ptu->right = pt;
...
void AvlTree::leftRotation(Node*& pt)
{
    ...
    if (ptu->balance == 1) // Inserção feita a direita, logo a rotação é simples a esquerda
    {
		//Rotação à esquerda
        pt->right = ptu->left;
        ptu->left = pt;
